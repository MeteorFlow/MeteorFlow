...
const { formDefinition, errorMsg, formBlocks, actions } = useFormStore();
console.log("dkjns", props.id);

if (errorMsg || !props.id) {
  await navigateTo("/forms");
}


onBeforeMount(async () => {
  actions.setSelectedVersionId(props.id as string)
